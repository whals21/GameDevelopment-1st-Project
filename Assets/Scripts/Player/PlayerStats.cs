    ...
    void Start()
    {
        ...
        if (File.Exists(path))
        {
            ...
            for (int i = 0; i < data.topRowLevel; i++)
            {
                ...
                switch (type)
                {
                    case 0: bonusDamage += 1.0f; break; // 공격력
                    case 1: bonusHp += 10.0f; break; // 체력
                    case 2: bonusRegen += 0.5f; break; // 체력 재생
                    case 3: bonusExp += 0.1f; break; // 경험치 (10%)
                }
            }

            // 아랫줄 계산 (이속 -> 쿨타임 반복)
            foreach (int specialId in data.purchasedSpecials)
            {
                // 100번 -> 이속, 101번 -> 쿨타임,
                int specialType = (specialId - 100) % 2;

                if (specialType == 0)
                {
                    bonusSpeed += 0.5f; // 이속 증가
                }
                else
                {
                    bonusCooldown += 0.05f; // 쿨타임 감소
                }
            }
            ...
            damage += bonusDamage;
            maxHp += bonusHp;
            healthRegenRate += bonusRegen;
            expBonusMultiplier += bonusExp;
            speed += bonusSpeed;
            cooldown -= bonusCooldown; // 감소
